#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Al-othmany Sender 3.2.1 — All-in-One
...
s = read(build_rel)
s2 = re.sub(r'versionCode\s*=\s*(?:310|320)\b', 'versionCode = 321', s, count=1)
s2 = re.sub(r'versionName\s*=\s*"(?:3\.1\.0|3\.2\.0)"', 'versionName = "3.2.1"', s2, count=1)
if s2 == s:
    if 'versionCode = 321' not in s or 'versionName = "3.2.1"' not in s:
        raise SystemExit("ERROR: Could not upgrade build.gradle.kts from 3.1.0/3.2.0 to 3.2.1")
else:
    write(build_rel, s2)
...
# ---------------------------------------------------------------------------
ar = r'''
    <!-- Al-othmany Sender 3.2.1 Secure Remote -->
    ...
    <string name="sender_failed_processing">تعذر المعالجة</string>
'''
en = r'''
    <!-- Al-othmany Sender 3.2.1 Secure Remote -->
    ...
    <string name="sender_session_counter_format">%1$d / %2$d</string>
    <string name="sender_failed_processing">Processing failed</string>
'''
...
# If 3.2.0 was already applied, the Secure marker exists and the two calls above are no-ops.
# Add the new 3.2.1 counter label independently so this all-in-one patch works from either base.
add_resources(
    "app/src/main/res/values/strings.xml",
    'name="sender_failed_processing"',
    '    <string name="sender_failed_processing">تعذر المعالجة</string>\n'
)
add_resources(
...
# ---------------------------------------------------------------------------
# 7B) Request result: X first -> verify -> Back fallback -> next link.
#     Counter wording is corrected.
# ---------------------------------------------------------------------------

# Shizuku: after a confirmed REQUEST_SUBMITTED result, use the detected X first.
# If X is absent or did not dismiss the sheet, Back is the immediate fallback.
# "Cancel request" is never used as the request-result exit action.
request_exit_anchor = '''        val safeCancel = snapshot.nodes.asSequence()
'''
request_exit_block = '''        if (snapshot.screenKind == AutomationScreenKind.REQUEST_SUBMITTED) {
...
'''
shizuku_text = read(shizuku_rel)
if "REQUEST_SENT_SHEET_BACK_FALLBACK" not in shizuku_text:
    if request_exit_anchor not in shizuku_text:
        raise SystemExit("ERROR: Shizuku request-result exit anchor not found")
    write(shizuku_rel, shizuku_text.replace(request_exit_anchor, request_exit_block, 1))

# FAILED is a broad processing bucket. It includes timeouts/unknown UI as well as open failures.
# Display a truthful name instead of calling every failure "could not open".
main_text = read(main_rel)
old_failed_line = '            failedCountText.text = "${stats.failed}\\n${getString(R.string.status_failed)}"\n'
new_failed_line = '            failedCountText.text = "${stats.failed}\\n${getString(R.string.sender_failed_processing)}"\n'
if "R.string.sender_failed_processing" not in main_text:
    if old_failed_line not in main_text:
        raise SystemExit("ERROR: failed counter label anchor not found in MainActivity")
    write(main_rel, main_text.replace(old_failed_line, new_failed_line, 1))
...
# ---------------------------------------------------------------------------
required = {
    "app/build.gradle.kts": ['versionCode = 321', 'versionName = "3.2.1"'],
...
print()
print("============================================================")
print(" Al-othmany Sender 3.2.1 all-in-one patch applied successfully")
print("============================================================")
print("Changed files:")
for rel in changed:
    print(" -", rel)
...
print(" - Request-sent flow = X first, Back fallback, then immediate next link")
print(" - Stable/Fast/Turbo/MAX/Custom selection is respected")
print(" - Faster selected-speed scrolling")
print(" - Session counter = completed / total; failed bucket renamed to Processing failed")
...
print("If both pass:")
print("  git add .")
print('  git commit -m "Al-othmany Sender 3.2.1 all-in-one secure x counter"')
