...
object RuntimeCadencePolicy {
    const val FAST_FALLBACK_POLL_MS = 35L
    const val NORMAL_FALLBACK_POLL_MS = 360L

    const val FAST_EVENT_SCAN_MS = 6L
    const val FAST_STABLE_SCAN_MS = 14L
    const val NORMAL_EVENT_SCAN_MS = 90L
    const val NORMAL_STABLE_SCAN_MS = 180L
    const val STABLE_SCREEN_RELAX_AFTER_SCANS = 4

    const val FAST_CLICK_THROTTLE_MS = 30L
    const val NORMAL_CLICK_THROTTLE_MS = 320L
    const val FAST_GESTURE_DURATION_MS = 10L
    const val NORMAL_GESTURE_DURATION_MS = 45L
    const val FAST_RESULT_INFERENCE_MS = 36L
    const val NORMAL_RESULT_INFERENCE_MS = 420L
    const val FAST_EXIT_SETTLE_MS = 8L
    const val NORMAL_EXIT_SETTLE_MS = 90L
    const val FAST_TERMINAL_SETTLE_MS = 18L
