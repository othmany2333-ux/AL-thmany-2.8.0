...
object ShizukuFastUiPolicy {
    const val EVENT_SCAN_MS = 14L
    const val STABLE_SCAN_MS = 36L
    const val FALLBACK_POLL_MS = 95L
    const val WATCHDOG_INTERVAL_MS = 40L

    const val CLICK_THROTTLE_MS = 68L
    const val GESTURE_DURATION_MS = 18L
    const val RESULT_ANALYSIS_FALLBACK_MS = 88L

    const val ACTION_RETRY_AFTER_MS = 120L
    const val MAX_ACTION_ATTEMPTS = 2
    const val POST_JOIN_MIN_EVIDENCE_MS = 35L
    ...
    const val TERMINAL_ESCAPE_SETTLE_MS = 42L
    const val NORMAL_EXIT_SETTLE_MS = 24L
