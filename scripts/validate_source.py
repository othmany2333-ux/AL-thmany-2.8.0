...
checks = {
    ...
    "versionCode 282": "versionCode = 282" in build,
    "versionName 2.8.2": 'versionName = "2.8.2"' in build,
    ...
    "Shizuku compact parity event frame": "waitAndSnapshot" in shizuku_service and "EVENT_TREE_COALESCE_MS = 0L" in shizuku_fast_policy and "EVENT_SCAN_MS = 14L" in shizuku_fast_policy and "STABLE_SCAN_MS = 36L" in shizuku_fast_policy and "FALLBACK_POLL_MS = 95L" in shizuku_fast_policy,
    "Shizuku 2.7.4 parity cadence preset": all(token in shizuku_fast_policy for token in ["CLICK_THROTTLE_MS = 68L", "GESTURE_DURATION_MS = 18L", "RESULT_ANALYSIS_FALLBACK_MS = 88L", "ACTION_RETRY_AFTER_MS = 120L", "POST_JOIN_MIN_EVIDENCE_MS = 35L", "NON_LOADING_WATCHDOG_MS = 1_000L", "UNKNOWN_TIMEOUT_MS = 2_000L", "LOADING_TIMEOUT_MS = 20_000L", "USER_INSTANT_ADVANCE_SETTLE_MS = 0L"]),
    ...
    "Shizuku 2.7 adaptive fast compatibility": all(token in shizuku_service for token in ["fastUiMode = FastUiMode.UNKNOWN", "/system/bin/uiautomator dump --compressed", "COMMAND_DUMP_COMPAT_RETRY_MS = 60L", "SHIZUKU_UI_TREE_PRESERVE_CURRENT", "currentPreserved=true"]),
    "Shizuku exact-user activity join proof": all(token in shizuku_service for token in ["probeJoinedConversationActivity", "SHIZUKU_ACTIVITY_JOIN_PROOF", "ShizukuActivityProofPolicy", "ACTIVITY_PROBE_TIMEOUT_MS", "ACTIVITY_PROBE_ATTEMPTS = 4", "ACTIVITY_PROBE_RETRY_MS = 55L"]) and (JAVA / "com/althmany/groupmanager/domain/ShizukuActivityProofPolicy.kt").exists(),
    ...
    "Shizuku successful join closes before next": "SHIZUKU_RESULT_SURFACE_CLOSE" in shizuku_service and "nextDirect=true" in shizuku_service,
    ...
    "verified terminal escape retries": "FAST_MAX_EXIT_STEPS = 3" in service and "FAST_TERMINAL_SETTLE_MS = 42L" in runtime_cadence,
    ...
    "adaptive event-first cadence": all(token in runtime_cadence for token in ("FAST_FALLBACK_POLL_MS = 95L", "FAST_EVENT_SCAN_MS = 14L", "FAST_STABLE_SCAN_MS = 36L", "FAST_CLICK_THROTTLE_MS = 68L")) and "RuntimeCadencePolicy.minScanIntervalMs" in service,
    ...
    "fast post-join evidence age": "FAST_POST_JOIN_MIN_EVIDENCE_AGE_MS = 45L" in (JAVA / "com/althmany/groupmanager/domain/InvitationStabilityPolicy.kt").read_text(encoding="utf-8"),
