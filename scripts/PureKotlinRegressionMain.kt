...
fun main() {
    ...
    expect("shizuku active click throttle", 68L, ShizukuRuntimePolicy.inputCooldownMs(true))
    expect("fast ui event scan", 14L, ShizukuFastUiPolicy.EVENT_SCAN_MS)
    expect("fast ui stable scan", 36L, ShizukuFastUiPolicy.STABLE_SCAN_MS)
    expect("fast ui fallback poll", 95L, ShizukuFastUiPolicy.FALLBACK_POLL_MS)
    expect("fast ui watchdog interval", 40L, ShizukuFastUiPolicy.WATCHDOG_INTERVAL_MS)
    expect("fast ui watchdog hard limit", 1_000L, ShizukuFastUiPolicy.NON_LOADING_WATCHDOG_MS)
    expect("fast ui action retry", 120L, ShizukuFastUiPolicy.ACTION_RETRY_AFTER_MS)
    expect("fast ui post join evidence", 35L, ShizukuFastUiPolicy.POST_JOIN_MIN_EVIDENCE_MS)
